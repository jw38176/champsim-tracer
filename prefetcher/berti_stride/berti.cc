...
namespace {
// IP-stride tracking structures
struct stride_tracker {
  struct stride_entry {
  ...
  champsim::msl::lru_table<stride_entry> table{TRACKER_SETS, TRACKER_WAYS};
...
std::map<CACHE*, stride_tracker> stride_trackers;
} // namespace
...
void CACHE::prefetcher_initialize() 
{
  ...
  std::cout << "Berti+IP-Stride Prefetcher" << std::endl;
...
{
  ::stride_trackers[this].advance_lookahead(this);
}
                                         ...
                                         uint8_t type, uint32_t metadata_in)
{
  ...
  // Initialize IP-stride tracking
  // Prefetch from stride, then from berti 
  ::stride_trackers[this].initiate_lookahead(ip, line_addr);
...
# endif
    ...
    if (prefetch_line(p_addr, fill_this_level, 0x1)) // 0x1 is the metadata for berti prefetch 
    {
                                      ...
                                      uint32_t metadata_in)
{
  ...
  uint64_t evicted_lineaddr = (evicted_addr >> LOG2_BLOCK_SIZE);
